<%-- 
    Document   : openISTRequestForApproval
    Created on : 29 Dec, 2020, 5:08:20 PM
    Author     : amols
--%>
...
String user_Id  = checkNull((String) request.getParameter("userid"));
*/
...
*/
System.out.println("#### openISTRequestForApproval.jsp");
String decodedUserId = checkNull((String) request.getParameter("userid"));
String userId = fileEnc.Decrypt(decodedUserId);
String decodedISTid = checkNull((String) request.getParameter("requestId"));
String requestId = fileEnc.Decrypt(decodedISTid);    
System.out.println("#### userId ["+userId+"]");

boolean isProcessed = false;
lrmTrans.makeConnection();
isProcessed = lrmTrans.isRecordProcessed(requestId,userId);
lrmTrans.breakConnection();
System.out.println("#### isProcessed ["+isProcessed+"]");

String role = "", fullname = "";
lrmTrans.makeConnection();
String userDetails = lrmTrans.getUserDetails(userId);
lrmTrans.breakConnection();
...
if((userId == null || userId.equals("-1") || userId.equals("")) && (role == null || role != "FM") && (requestId == null)) 
{  
...
<div class="container">
    ...
    <div class="alert alert-success" style="text-align: center;max-width: 420px;margin: 15px auto 0 auto;">
            This IST Details is Already Processed 
        </div>
    <% return;
    }%>
...
<%
    ...
    String appType = fileEnc.Decrypt(checkNull((String) request.getParameter("appType")));   
    ...
    if(rs.next())
    {
        ...
        {            
            lrmTrans.makeConnection();
            nextLevel = lrmTrans.getNextLevel(requestId);
            lrmTrans.breakConnection();
            
            lrmTrans.makeConnection();
            pendingUser =  lrmTrans.getPendingUser(requestId,nextLevel,istStatus,transferorSchemeNo,transfereeSchemeNo,"linkview");
            lrmTrans.breakConnection();
        }
        ...
        lrmTrans.makeConnection();
        sellerCategoryRs = lrmTrans.getCategories(transferorSchemeNo);
        ...
        lrmTrans.makeConnection();
        buyerCategoryRs = lrmTrans.getCategories(transfereeSchemeNo);
    ...
    <table class="table" style="width:75%" align="center">
       ...
       <!--Part 3 END-->
        <h3  class="heading-bg">Select Your Action</h3>    
    <div class="row form-group">
      
     <%if("Approve".equals(aprrovestatus)){%>
        <div class="col-md-4 col-sm-6">           
            <input type="radio" id="approveRrk" name="approveRejectRadio" value="Approve" class="radio-btn" checked>
            <label>Approve</label>
        </div>
       <div class="col-md-4 col-sm-6">
            <input type="radio" id="rejectRrk" name="approveRejectRadio" value="Reject" class="radio-btn">
            <label>Reject</label>
        </div>
        <% }else if(("Reject").equals(aprrovestatus)){%>
        <div class="col-md-4 col-sm-6">          
            <input type="radio" id="approveRrk" name="approveRejectRadio" value="Approve" class="radio-btn">
            <label>Approve</label>
        </div>
        <div class="col-md-4 col-sm-6">
            <input type="radio" id="rejectRrk" name="approveRejectRadio" value="Reject" class="radio-btn" checked>
             <label>Reject</label>
         </div>
        <%}%>   
    <div class="col-md-4 col-sm-6">
        <textarea maxlength="1000" class="form-control " style="width: 400px;" rows="4" cols ="50" id="approveRejectRemark" name="approveRejectRemark"></textarea>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-md-12 text-center">          
        <input class="btn btn-primary" type="button" onClick="approveRejectRemark('<%=requestId%>','<%=role%>','<%=appType%>','<%=istType%>')" value="Submit">  
